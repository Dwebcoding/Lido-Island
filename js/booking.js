...
function sendBookingEmail(booking) {
    ...
    // Se EmailJS non è caricato, salta l'invio email
    if (typeof emailjs === 'undefined') {
        console.warn('[EmailJS] ⚠️ EmailJS non disponibile. Email non inviata, ma prenotazione salvata.');
        return;
    }
    ...
    try {
        ...
        console.log('[EmailJS] Parametri template:', templateParams);
        ...
        emailjs.send(
            ...
            logBooking('Email inviata con successo', {
                status: response.status,
                text: response.text,
                booking_id: booking.id
            });
            ...
            logBooking('❌ Errore nell\'invio email', error);
        });
    } catch (error) {
        console.error('[EmailJS] ❌ ERRORE CRITICO nell\'invio email:', error);
    }
}
